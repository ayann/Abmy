...
public class TetrisView implements Disposable {
    ...
    public TetrisView(TetrominoManager tetrominoManager, TetrisController tetrisController, float gameWidth, float gameHeight) {
    ...
    private void renderScore() {
        ...
        String score = String.valueOf(tetrominoManager.score);
        ...
        String level = String.valueOf(tetrominoManager.level);
        //textWidth = scoreFont.getBounds(level).width;
        scoreGlyphLayout.setText(scoreFont, level);
        textWidth = scoreGlyphLayout.width;
        textX = marginRightCenterX - textWidth / 2;
    ...
    public synchronized void renderPlayfield() {
        ...
        for (int i = 2; i < tetrominoManager.playfield.length; i++) {
            for (int j = 0; j < tetrominoManager.playfield[0].length; j++) {
                if (tetrominoManager.playfield[i][j] > 0) {
                    ...
                    drawBlock(tetrominoManager.playfield[i][j], x, y);
                }
            }
        }
    }
